...
def hasAds(msg):
	links = list(map(lambda ID: ID.strip()[1:],findall("http://","https://",".ir",".com",".org",".net",".me", msg))) + list(map(lambda link:link.split("/")[-1],findall("http://","https://",".ir",".com",".org",".net",".me",msg)))
...
g_usvl = ''
test_usvl = ''
auth = "hqmpzihkcnpkqqipiowxjnffvvlyondk"
bot = Bot(auth)
list_message_seened = []
